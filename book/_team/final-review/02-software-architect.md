...
- **Suggested fix:** "At `book-m6-final` (commit `d41e2b8`) the repository has 38 commits and pull requests 1 to 12; a later documentation-only pull request (#13) corrected the README's mention of a canvas. At the tag, trust the code (Chapter 21), not that sentence."
...
- The counts 114 backend and 31 frontend tests were not reproduced (`@Test` count at `f1bb3a8` is 116 by grep, which includes annotations that may not all be executed; frontend `it(` count is 31).
