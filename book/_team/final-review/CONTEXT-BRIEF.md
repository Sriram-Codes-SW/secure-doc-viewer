...
Quick checks: `git tag | grep book` lists the seven tags; the final app commit is `d41e2b8`. Listing exactness can be checked by comparing a fenced block with `git show <tag>:<path>`; an earlier script `listingcheck.py` did this for some chapters (its location is unverified).
