...
**The janitor cleans what remains.** Some failures can't be undone at the moment they happen (a crash between committing tiles and saving the row, or a delete that hits a locked folder). `StorageJanitor` (Chapter 14) sweeps directories nothing points to. It is deliberately conservative: it touches only directories whose names look like document ids, only ones older than an hour, and it keeps all versions of a document whose *current* version is missing from disk, because then the others may be the only copy left.
