...
| Caddy (TLS) and the published port | **Elastic Load Balancing**, an **Application Load Balancer (ALB)**, with a certificate from **AWS Certificate Manager (ACM)**, and a **Route 53** DNS record | 6 | the ALB adds no HSTS unless configured (this design keeps it in nginx), and it sees a different client address |
...
`SessionKeys`, `SessionLifetimeFilter` and the cookie-based CSRF token repository need no change.
...
- **Keeping the in-memory throttles, or putting a CDN in front of tiles.** Symptoms: limits that multiply with the number of tasks; unsharing that stops working on open pages, or watermarks that vanish. Fix: shared, atomic counters, and tiles that stay app-served and are never cached at a shared edge (Chapter 41, section 41.8).
